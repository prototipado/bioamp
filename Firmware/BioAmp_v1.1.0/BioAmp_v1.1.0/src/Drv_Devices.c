...
void Drv_Devices_boardReset(void) {
	...
	char vstring [2];
    Drv_ADS1299_initialize(); // Initializes accelerometer and on-board ADS and on-daisy ADS if present
    Drv_UART_Send((uint8_t*)("BioAmp V1.1.0 8-16 canales\r\n"),28);
    Drv_UART_Send((uint8_t*)("Serial 018-002\r\n"),18); // Change accordingly
    ...
    Drv_UART_Send((uint8_t*)("Firmware: v1.1.0"),17);
    Drv_Devices_printMenu();
  ...
  */
  void Drv_Devices_sendChannelData()
  {
	  if(is_running) // if the ADS is acquiring data
	  {
		  ...
		  uint8_t auxData[6]; //array to get auxiliary data from Master
		  uint8_t i = 0; //loop counter
		  ...
		  gpio = boardStat & 0b00001111;
		  if(gpio&0b00000001)
		  {auxData[0] = 0; auxData[1] = 0;}
		  else{auxData[0] = 0; auxData[1] = 1;}
		  if(gpio&0b00000010)
		  {auxData[2] = 0; auxData[3] = 0;}
		  else{auxData[2] = 0; auxData[3] = 1;}
		  if(gpio&0b00000100)
		  {auxData[4] = 0; auxData[5] = 0;}
		  else{auxData[4] = 0; auxData[5] = 1;}

		  for(i=0; i<6; i++){
		  	  Drv_UART_Send(&auxData[i],1);
		  }
  ...
  void SysTick_Handler(void)
  {
	  ...
	  if(isMaster == FALSE)
	  {
	//	  Drv_GPIO_setPinToggle(LED_R_PORT, LED_V_PIN);
	  }
	  else
	  {
	//	  Drv_GPIO_setPinToggle(LED_V_PORT, LED_R_PIN);
	  }
